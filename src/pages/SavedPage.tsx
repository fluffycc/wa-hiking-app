...
export function SavedPage() {
  const { trails, loadTrails } = useTrailStore()
        ...
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            {[
              ...
              <button key={key} onClick={() => setPass(key, !myPasses[key])}
                className={`text-sm font-body font-medium rounded-full px-3 py-1.5 border transition-all
                  ${myPasses[key] ? 'bg-trail-green text-white border-trail-green' : 'bg-white text-trail-dark border-gray-200'}`}>
                {myPasses[key] ? '✓ ' : ''}{label}
              </button>
            ))}
          </div>
        ...
        {savedTrails.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-trail-stone font-body mt-1 max-w-xs">
              Tap the heart on any trail to save it here.
            </p>
          </div>
        ) : (
          <>
